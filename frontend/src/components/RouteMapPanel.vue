...
const replayLabel = computed(() => {
  if (props.latestIndex <= 0) {
    return "至少需要 2 次采样后才能回放";
  }
  return `当前第 ${props.activeIndex + 1} / ${props.latestIndex + 1} 个采样点`;
});
</script>
  ...
  <section class="panel panel-map">
    <div class="panel-head">
      ...
      <span class="pill" :class="routeTone">
        {{ replayActive ? `轨迹回放中 ${activeIndex + 1}/${Math.max(trajectory.length, 1)}` : trajectory.length ? `已采样 ${trajectory.length} 次` : "等待采样" }}
      </span>
    </div>
    ...
    <div class="map-stage">
      <svg v-if="trajectory.length && activePoint && latestPoint" viewBox="0 0 720 380" aria-label="车辆轨迹视图">
        ...
        <polyline :points="routePath" fill="none" stroke="url(#trajectoryGlow)" stroke-width="7" stroke-linecap="round" stroke-linejoin="round" opacity="0.24" />
        ...
        <g v-for="(point, index) in mappedPoints" :key="`${point.sampleTime}-${index}`">
          <circle
            ...
            :fill="index === activeIndex ? '#ffb347' : '#57d3ff'"
            :opacity="index === activeIndex || index === latestIndex ? 1 : 0.72"
          />
        </g>

        <g>
          <circle :cx="latestPoint.x" :cy="latestPoint.y" r="18" fill="rgba(25,211,166,0.2)" />
          <circle :cx="latestPoint.x" :cy="latestPoint.y" r="10" fill="#19d3a6" filter="url(#traceGlow)" />
          <text :x="latestPoint.x + 16" :y="latestPoint.y + 5" fill="#eff7fa" font-size="14">最新位置</text>
        </g>

        <g>
          <circle :cx="mappedPoints[activeIndex].x" :cy="mappedPoints[activeIndex].y" r="14" fill="rgba(255,179,71,0.18)" />
          <circle :cx="mappedPoints[activeIndex].x" :cy="mappedPoints[activeIndex].y" r="8" fill="#ffb347" />
