...
<template>
  <section class="panel panel-chart">
    <div class="panel-head">
      ...
      <span class="pill" :class="temperatureTone">
        {{ activePoint?.trend || "等待数据" }}
      </span>
    </div>
    ...
    <div class="chart-stage">
      <svg v-if="chartData" :viewBox="`0 0 ${chartData.width} ${chartData.height}`" aria-label="温度趋势图">
        ...
        <rect :x="chartData.padding.left" :y="chartData.safeTop" :width="chartData.width - chartData.padding.left - chartData.padding.right" :height="chartData.safeHeight" fill="rgba(25,211,166,0.09)" />
        ...
        <polyline :points="chartData.polyline" fill="none" stroke="#57d3ff" stroke-width="4" stroke-linejoin="round" stroke-linecap="round" />
        ...
        >
          <circle
            ...
            :fill="index === activeIndex ? '#19d3a6' : '#07151d'"
            :stroke="index === activeIndex ? '#19d3a6' : '#57d3ff'"
            stroke-width="3"
          />
    ...
    <div class="focus-card">
      <div>
        ...
        <small>
          {{
            activePoint
              ? `湿度 ${formatNumber(activePoint.humidity)}% · 速度 ${formatNumber(activePoint.speed)} km/h · ${activePoint.doorOpen ? "车门开启中" : "车门关闭"} · 后端记录 ${activePoint.recordTime}`
              : "点击折线上的采样点，可联动查看该时刻的车辆轨迹位置和运行指标。"
          }}
        </small>
      </div>
